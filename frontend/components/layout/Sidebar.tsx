...
export function Sidebar({ onNavigate }: SidebarProps) {
  ...
  const activeGroupLabel = useMemo(() => {
    return navGroups.find(({ items }) =>
      items.some(({ href }) => pathname === href || pathname.startsWith(href + "/"))
    )?.label;
  }, [pathname]);
          ...
          return (
            <div key={label}>
              <button
                ...
                className="mb-2 flex min-h-8 w-full items-center gap-1.5 rounded-md px-1.5 text-left text-[10px] font-semibold uppercase tracking-wide text-notion-muted transition-colors hover:bg-notion-bg3 hover:text-notion-text md:mb-1 md:min-h-6 md:px-1"
              >
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-notion-bg3 text-notion-muted md:h-5 md:w-5">
                  <ChevronRight
                    className={cn(
